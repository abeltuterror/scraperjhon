...
async def setup_database():
    """Crea la base y la tabla SIN la restricción UNIQUE."""
    ...
    # Tabla SIN UNIQUE (insertará siempre)
    await conn.execute("""
        CREATE TABLE IF NOT EXISTS convocatorias6 (
    ...
    """)
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_ubicacion ON convocatorias6(ubicacion)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_remuneracion ON convocatorias6(remuneracion)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_terminal_estado ON convocatorias6(terminal, estado)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_datos_completos ON convocatorias6 USING GIN (datos_completos)")
    await conn.close()
...
async def main():
    ...
    # Opcional: borrar registros anteriores de este terminal (para empezar limpio)
    # await conn.execute("DELETE FROM convocatorias6 WHERE terminal = $1", terminal)
