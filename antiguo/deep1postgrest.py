...
DB_CONFIG = {
    ...
    "database": os.getenv("DB_NAME")  # 'postgres' por defecto
}
DB_SCRAPING_NAME = os.getenv("DB_SCRAPING_NAME", "scraping")
...
async def click_seguro(locator, timeout=15000, force=False):
    """Hace clic en un elemento, esperando que no haya overlay bloqueante."""
    try:
...
async def guardar_vacante(conn, vacante, terminal, scraping_id, estado='exitoso', error_msg=None):
    """Insertar o actualizar una vacante en PostgreSQL."""
...
async def main():
    ...
    # Conectar a la base de datos scraping
    config = DB_CONFIG.copy()
    config["database"] = DB_SCRAPING_NAME
    conn = await asyncpg.connect(**config)
