...
# ========== NUEVAS FUNCIONES PARA CONTROL DE FOLIO ==========
async def extraer_folio(page):
    """
    Extrae exclusivamente el número de folio de la página de detalle.
    Retorna el folio como string o None si no se encuentra.
    """
    try:
        ...
        print(f"      ⚠️ Error extrayendo folio (rápido): {e}")
...
async def folio_ya_existe(conn, folio):
    """Verifica si ya existe un registro con el folio dado en la tabla convocatorias."""
    if not folio:
        return False
    result = await conn.fetchval("""
        SELECT 1 FROM convocatorias
        WHERE detalles_json->>'NUMERO_FOLIO' = $1
        LIMIT 1
    """, str(folio))
    return result is not None

def es_registro_valido(detalles):
    """Descarta registros sin los campos mínimos para ser útiles."""
    puesto = detalles.get("PUESTO", "").strip()
    entidad = detalles.get("ENTIDAD_AVISO", "").strip()
    folio = detalles.get("NUMERO_FOLIO", "").strip()
    return bool(puesto and entidad and folio)

# ========== FUNCIONES EXISTENTES (extraer_ubicacion_tarjeta, extraer_detalles_completos) ==========
...
async def extraer_detalles_completos(page):
    ...
    # 1. Número de folio (ahora también se extrae aquí, pero si ya lo tenemos no es necesario)
    try:
        folio_span = page.locator("div.cuadro-seccion-lat span.sub-titulo-2")
        if await folio_span.count():
            texto = limpiar(await folio_span.first.inner_text())
...
# ========== SETUP DE BASE DE DATOS MODIFICADO (con índice adicional) ==========
async def setup_database():
    ...
    """)
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_detalles_json ON convocatorias USING GIN (detalles_json)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_terminal_estado ON convocatorias(terminal, estado)")
    # NUEVO ÍNDICE para búsqueda rápida por NUMERO_FOLIO dentro del JSONB
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_folio ON convocatorias ((detalles_json->>'NUMERO_FOLIO'))")
    await conn.close()
    print("📦 Tabla 'convocatorias' e índices listos.")

async def guardar_detalle(conn, detalles, terminal, scraping_id, estado='exitoso', error_msg=None):
    await conn.execute("""
        INSERT INTO convocatorias (detalles_json, terminal, scraping_id, estado, error)
        VALUES ($1, $2, $3, $4, $5)
    """,
...
# ========== SCRAPE_RANGO MODIFICADO: COMPROBACIÓN DE FOLIO ANTES DE EXTRAER ==========
    ...
    async with async_playwright() as p:
        ...
        while pagina_actual <= MAX_PAGINAS:
            ...
            for idx in range(inicio_idx, min(fin_idx, total_botones)):
                ...
                try:
                    # Click en "Ver más" para abrir detalle
                    boton = page.locator("span.ui-button-text.ui-c:has-text('¡Ver más!')").nth(idx)
                    await click_seguro(boton)
                    await page.wait_for_load_state("networkidle")

                    # ---- NUEVO: Extraer folio y comprobar si ya existe ----
                    folio = await extraer_folio(page)
                    if folio and await folio_ya_existe(conn, folio):
                        print(f"      ⚠️ Folio {folio} ya existe en BD. Saltando esta vacante.")
                        await retroceder(page)
                        continue   # Saltar a siguiente botón sin extraer más

                    # Si el folio no existe (o no se pudo obtener), procedemos con extracción completa
                    detalles = await extraer_detalles_completos(page)

                    # Retroceder para extraer ubicación desde la tarjeta
                    ...
                    # Guardar en BD solo si tiene los campos mínimos
                    if es_registro_valido(detalles):
                        ...
                        print(f"      ⚠️ Registro inválido (sin PUESTO/ENTIDAD/FOLIO), se omite.")
...
async def main():
    ...
    print(f"💾 Datos guardados en PostgreSQL base '{DB_SCRAPING_NAME}', tabla 'convocatorias'")
