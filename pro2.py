...
MAX_PAGINAS = 2  # límite de seguridad
...
async def extraer_detalles_completos(page):
    """Extrae los campos adicionales tras hacer clic en 'Ver más'."""
    detalles = {}
...
async def setup_database():
    """Crea la base y la tabla sin restricción UNIQUE."""
...
async def guardar_vacante(conn, vacante, terminal, scraping_id, estado='exitoso', error_msg=None):
    """Inserta siempre (sin conflict checking)."""
...
async def scrape_paginas(inicio, paso, terminal, scraping_id):
    """
    Función que procesa páginas de la lista.
    Recibe una página de inicio y un paso (1, 2, 3...).
    Ejemplo: inicio=1, paso=2 → páginas 1,3,5,...
    """
    todas_vacantes = []  # solo para esta tarea
    config = DB_CONFIG.copy()
    config["database"] = DB_SCRAPING_NAME
    conn = await asyncpg.connect(**config)
    ...
    async with async_playwright() as p:
        ...
        while not sin_mas_vacantes and pagina_actual <= MAX_PAGINAS:
            ...
            for idx, tarjeta in enumerate(tarjetas):
                ...
                # Guardado incremental JSON (cada tarea sobre su archivo)
...
async def main():
    ...
    # Lanzamos dos tareas en paralelo, cada una con páginas alternas
    # Tarea 1: páginas impares (1,3,5...)
    # Tarea 2: páginas pares (2,4,6...)
    ...
    # Unimos todos los resultados (opcional)
