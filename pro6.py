...
async def scrape_rango(inicio_idx, fin_idx, terminal, scraping_id):
    """
    Procesa todas las páginas. En cada página, solo las vacantes con índice en [inicio_idx, fin_idx).
    Busca el botón 'Ver más' dentro de la propia tarjeta para evitar desorden.
    """
    todas_vacantes = []
    config = DB_CONFIG.copy()
    config["database"] = DB_SCRAPING_NAME
    ...
    async with async_playwright() as p:
        ...
        while pagina_actual <= MAX_PAGINAS:
            ...
            # Procesar solo el rango de índices que corresponden a esta tarea
            for idx in range(inicio_idx, min(fin_idx, total)):
                # Obtener la tarjeta del índice correcto
                tarjeta_soup = tarjetas[idx]
                # Extraer datos de la tarjeta (sin usar Playwright para la tarjeta, porque ya tenemos el soup)
                titulo_elem = tarjeta_soup.find("div", class_="titulo-vacante")
                titulo = limpiar(titulo_elem.find("label").text) if titulo_elem and titulo_elem.find("label") else "N/A"
                entidad_elem = tarjeta_soup.find("div", class_="nombre-entidad")
                entidad = "N/A"
                if entidad_elem:
                ...
                datos_tarjeta = {}
                for fila in tarjeta_soup.find_all("div", class_="row box-mb"):
                ...
                # Ahora localizar la tarjeta en el DOM de Playwright para hacer clic en su botón
                # Necesitamos obtener una referencia a la tarjeta Playwright mediante su posición
                # Usamos page.locator(...).nth(idx) para seleccionar la tarjeta por índice
                tarjeta_playwright = page.locator("div.cuadro-vacantes").nth(idx)
                boton = tarjeta_playwright.locator("span.ui-button-text.ui-c:has-text('¡Ver más!')")
                if await boton.count():
                    print(f"   [Rango {inicio_idx}-{fin_idx}] 🔹 Vacante {idx+1}/{total} - Haciendo clic...")
                    await click_seguro(boton)
                    await page.wait_for_load_state("networkidle")
                    # Pequeña pausa para que cargue el modal
                    ...
                    # Si no tiene botón, guardar solo datos de tarjeta
                    vacante["Titulo_Aviso"] = vacante["Institucion_Aviso"] = "N/A"
...
async def main():
    ...
    # Rangos: (0,2), (2,4), (4,6), (6,8), (8,10) → cubren índices 0 a 9 (10 vacantes por página)
