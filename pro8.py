...
NUM_WORKERS = 5          # Número de workers en paralelo
MAX_PAGINAS_POR_WORKER = 2   # None = todas las páginas, o un entero (ej. 2)
...
async def worker(worker_id, offset, step, terminal, scraping_id):
    """
    Worker que procesa páginas: offset, offset+step, offset+2*step, ...
    """
    todas_vacantes = []
    config = DB_CONFIG.copy()
    config["database"] = DB_SCRAPING_NAME
    ...
    async with async_playwright() as p:
        ...
        print(f"🔄 Worker {worker_id} (offset={offset}, step={step}) iniciando...")
        await page.goto(URL)
        await page.wait_for_load_state("networkidle")

        # Navegar hasta la página offset (primera página que le toca)
        pagina_actual = 0   # 0 = página 1
        while pagina_actual < offset:
            try:
                ...
                if await next_btn.count() and await next_btn.is_visible() and await next_btn.is_enabled():
                    ...
                    print(f"Worker {worker_id}: No se pudo llegar a la página {offset+1}, terminando.")
                    await browser.close()
                    return []
            except Exception as e:
                print(f"Worker {worker_id}: error navegando a página {offset+1}: {e}")
                await browser.close()
                return []
        ...
        while True:
            if MAX_PAGINAS_POR_WORKER is not None and paginas_procesadas >= MAX_PAGINAS_POR_WORKER:
                break
            print(f"\n📄 Worker {worker_id} procesando página {pagina_index+1}")
            await page.wait_for_selector("div.cuadro-vacantes", timeout=15000)
            tarjetas = await page.locator("div.cuadro-vacantes").all()
            total = len(tarjetas)
            if total == 0:
                break
            print(f"   Worker {worker_id}: {total} vacantes en página {pagina_index+1}")
            for idx, tarjeta in enumerate(tarjetas):
                ...
                # Guardado incremental por worker
                with open(f"worker_{worker_id}_progress.json", "w", encoding="utf-8") as f:
                    json.dump(todas_vacantes, f, ensure_ascii=False, indent=2)

            # Avanzar 'step' páginas
            for _ in range(step):
                try:
                    next_btn = page.locator("button:has-text('Sig.')").last
                    if await next_btn.count() and await next_btn.is_visible() and await next_btn.is_enabled():
                        ...
                        print(f"Worker {worker_id}: no se pudo avanzar más páginas. Terminando.")
                        await browser.close()
                        return todas_vacantes
                except Exception as e:
                    print(f"Worker {worker_id}: error al avanzar página: {e}")
                    await browser.close()
                    return todas_vacantes
...
async def main():
    ...
    print(f"🚀 Lanzando {NUM_WORKERS} workers en paralelo (step={NUM_WORKERS})")

    # Crear tareas: offset = 0,1,2,..., NUM_WORKERS-1
    tasks = []
    for offset in range(NUM_WORKERS):
        tasks.append(asyncio.create_task(worker(offset, offset, NUM_WORKERS, terminal, scraping_id)))
    resultados = await asyncio.gather(*tasks)

    total_vacantes = sum(len(r) for r in resultados)
    print(f"\n✅ Total de vacantes extraídas: {total_vacantes}")
    for i, res in enumerate(resultados):
        print(f"   Worker {i}: {len(res)} vacantes")
