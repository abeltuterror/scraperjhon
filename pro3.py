...
DELAY_SEGUNDOS = 10.0         # Pausa base en segundos (puedes aumentarlo a 2 o 3)
RANDOM_EXTRA = 0.5            # Variación aleatoria ±0.5 segundos
# ============================================

async def pausa_corta():
    """Espera un tiempo aleatorio para simular comportamiento humano."""
    espera = DELAY_SEGUNDOS + random.uniform(-RANDOM_EXTRA, RANDOM_EXTRA)
    if espera < 0.2:
        espera = 0.2
...
async def retroceder(page):
    try:
        ...
        await pausa_corta()  # Pausa después de retroceder
        return True
...
async def scrape_paginas(inicio, paso, terminal, scraping_id):
    todas_vacantes = []
    config = DB_CONFIG.copy()
    config["database"] = DB_SCRAPING_NAME
    conn = await asyncpg.connect(**config)
    ...
    async with async_playwright() as p:
        ...
        print(f"📍 [Tarea {inicio}] Navegando a {URL}...")
        await page.goto(URL)
        await page.wait_for_load_state("networkidle")
        await pausa_corta()  # Pausa inicial

        # Navegamos hasta la página de inicio
        pagina_actual = 1
        while pagina_actual < inicio:
            try:
                ...
                if await next_btn.count() and await next_btn.is_visible() and await next_btn.is_enabled():
                    ...
                    await page.wait_for_load_state("networkidle")
                    await pausa_corta()   # Pausa tras avanzar página
                    pagina_actual += 1
                else:
                    break
            except:
        ...
        while not sin_mas_vacantes and pagina_actual <= MAX_PAGINAS:
            print(f"\n📄 [Tarea {inicio}] Procesando página {pagina_actual}")
            await page.wait_for_load_state("networkidle")
            ...
            if not tarjetas:
                print(f"   [Tarea {inicio}] No hay más vacantes.")
                break

            print(f"   [Tarea {inicio}] Se encontraron {len(tarjetas)} vacantes")
            botones_ver_mas = await page.locator("span.ui-button-text.ui-c:has-text('¡Ver más!')").all()

            for idx, tarjeta in enumerate(tarjetas):
                print(f"   [Tarea {inicio}] 🔹 Vacante {idx+1}/{len(tarjetas)}")
                ...
                try:
                    if idx < len(botones_ver_mas):
                        print(f"      [Tarea {inicio}] 👆 Click en 'Ver más'...")
                        await click_seguro(botones_ver_mas[idx])
                        await page.wait_for_load_state("networkidle")
                        await pausa_corta()   # Pausa tras abrir modal
                        ...
                        print(f"      [Tarea {inicio}] ✅ Extraídos: {list(campos_extra.keys())}")
                        await retroceder(page)   # retroceder ya incluye pausa
                    else:
                        vacante["Titulo_Aviso"] = vacante["Institucion_Aviso"] = "N/A"

                    await guardar_vacante(conn, vacante, terminal, scraping_id, estado='exitoso')
                except Exception as e:
                    print(f"      [Tarea {inicio}] ❌ Error: {e}")
                ...
                with open(f"prueba_tarea_{inicio}.json", "w", encoding="utf-8") as f:
                    json.dump(todas_vacantes, f, ensure_ascii=False, indent=2)

                await pausa_corta()   # Pausa entre vacantes de la misma página

            # Avanzar según el paso (páginas saltadas)
            avanzar = paso
            while avanzar > 0:
                try:
                    next_btn = page.locator("button:has-text('Sig.')").last
                    if await next_btn.count() and await next_btn.is_visible() and await next_btn.is_enabled():
                        await click_seguro(next_btn)
                        await page.wait_for_load_state("networkidle")
                        await pausa_corta()   # Pausa tras cambiar de página
                        pagina_actual += 1
                        avanzar -= 1
                    else:
...
async def main():
    ...
    # Lanzamos dos tareas en paralelo (páginas impares y pares)
    tarea1 = asyncio.create_task(scrape_paginas(1, 2, terminal, scraping_id))
    tarea2 = asyncio.create_task(scrape_paginas(2, 2, terminal, scraping_id))
    ...
    print(f"\n✅ Total de vacantes extraídas: {len(todas)} (Tarea1: {len(resultados1)}, Tarea2: {len(resultados2)})")
