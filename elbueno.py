...
MAX_PAGINAS = 1000  # Número de páginas a procesar
...
async def setup_database():
    ...
    await conn.execute("""
        CREATE TABLE IF NOT EXISTS convocatoriabien (
    ...
    """)
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_detalles_json ON convocatoriabien USING GIN (detalles_json)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_terminal_estado ON convocatoriabien(terminal, estado)")
    await conn.close()
    print("📦 Tabla 'convocatoriabien' e índices listos.")

async def guardar_detalle(conn, detalles, terminal, scraping_id, estado='exitoso', error_msg=None):
    await conn.execute("""
        INSERT INTO convocatoriabien (detalles_json, terminal, scraping_id, estado, error)
        VALUES ($1, $2, $3, $4, $5)
    """,
...
async def main():
    ...
    print(f"💾 Datos guardados en PostgreSQL base '{DB_SCRAPING_NAME}', tabla 'convocatoriabien'")
