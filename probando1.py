...
async def setup_database():
    """Crea la base de datos destino con UTF-8 y locale C.UTF-8, luego la tabla SIN UNIQUE."""
    ...
    # Crear tabla SIN la restricción UNIQUE
    await conn.execute("""
        CREATE TABLE IF NOT EXISTS convocatorias4 (
    ...
    """)
    # Índices (sin UNIQUE)
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_ubicacion ON convocatorias4(ubicacion)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_remuneracion ON convocatorias4(remuneracion)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_terminal_estado ON convocatorias4(terminal, estado)")
    await conn.execute("CREATE INDEX IF NOT EXISTS idx_datos_completos ON convocatorias4 USING GIN (datos_completos)")
    await conn.close()
...
async def guardar_vacante(conn, vacante, terminal, scraping_id, estado='exitoso', error_msg=None):
    """Inserta una vacante en PostgreSQL (sin conflicto, siempre inserta)."""
            ...
            for idx, tarjeta in enumerate(tarjetas):
                ...
                try:
                    if idx < len(botones_ver_mas):
                        ...
                        # XPath para título e institución
                        try:
                    ...
                    # Insertar en base de datos (siempre inserta)
                ...
                # Guardado incremental JSON (después de cada vacante)
